...
import { questions as fullQuestions } from "@/data/questions";
import { motion } from "framer-motion";

export default function CBTQuizApp() {
  const [questions, setQuestions] = useState(fullQuestions);
  ...
  const retryIncorrect = () => {
    const incorrect = results
      .map((r, i) => ({ ...r, ...fullQuestions.find(q => q.id === r.id) }))
      .filter((r) => !r.isCorrect);
    setQuestions(incorrect); // 👈 오답만 다시 questions로 설정
    setCurrent(0);
    setScore(0);
    setSelected(null);
    setFinished(false);
...
<a
  href="https://passthetest.tistory.com"
  target="_blank"
